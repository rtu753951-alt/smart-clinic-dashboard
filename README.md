...
- **前端邊緣運算**：在瀏覽器端即可完成複雜的交叉分析（醫師工時、到診率、主力療程營收分析），無需依賴龐大後端伺服器，保障資料隱私（包含臨床服務工時與行政結構性工時的分流建模）。
...
## 🧑‍⚕️ 6. Workforce Analytics Model（人力分析模型說明）

本系統將「人力工時」區分為兩種性質，以避免服務產能與行政負擔混合造成決策誤判：

### 6-1. 服務工時（Value-Add Workload）
代表實際投入於療程交付的臨床服務時間，計算方式為：

療程時長（services.csv） × 職務占比表 × 預約數量（appointments.csv）
...
### 6-2. 結構性工時（Hidden / Overhead Workload）
代表非直接療程服務之支援性工作時間，例如：
...
資料來源：
- staff_workload.csv → minutes 欄位

適用角色：
- admin（行政）
- 亦可包含臨床人員的非服務工時（如內部會議）
...
- 不參與療程服務工時分攤
- 不納入臨床負載熱力圖與 SOP 基準計算
- 僅於以下模組顯示：
  - 總覽 KPI（行政人數）
  - Hidden / Overhead 工時統計
  - 營運壓縮率（結構性負擔）

此設計用以確保：

> 臨床圖表反映「治療產能」，營運指標反映「支援成本」。
...
|----------|-----------|
| 服務結構分析 | 服務工時 |
| SOP 基準 vs 實際 | 服務工時 + 結構性工時 |
| 人力負載熱力圖 | 僅服務工時 |
| KPI / 壓縮率 | 結構性工時 |

### 🎮 7. 營運情境模擬器 (Operational Sandbox Mode) [ADVANCED]
...
- **決策連動分析**：
 ...
 - **人力負載預測**：即時計算調整後的臨床服務工時與結構性工時負載，並估算「隱性疲勞風險」。
